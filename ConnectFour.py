...
class Game:
    def __init__(self, player1, player2, time):
        ...
        self.game_has_human = self.players[0].type == 'human' or self.players[1].type == 'human'
        ...
        self.c = tk.Canvas(root, width=800, height=650)
        self.c.pack()

        for row in range(0, 700, 100):
            column = []
            for col in range(0, 600, 100):
                column.append(self.c.create_oval(
                    row+50, col+25, row+150, col+125, fill=''))
            self.gui_board.append(column)

        if not self.game_has_human:
            self.button = tk.Button(
                root, text='Next Move', command=self.make_move)
            self.button.pack()
    ...
    def make_move(self):
        if not self.game_over:
            ...
            if self.game_completed(current_player.player_number):
                self.game_over = True
                wintext = self.players[self.current_turn].player_string + ' wins!'
                self.player_string.configure(text=wintext)
                print(wintext)
                not self.game_has_human and self.button.configure(
                    state=DISABLED)
            else:
                self.current_turn = int(not self.current_turn)
                self.player_string.configure(
                    text=self.players[self.current_turn].player_string)
                if(self.game_has_human):
                    self.tk.update()
                    self.make_move()
...
def main(player1, player2, time, debug):
    def make_player(name, num):
        if name == 'ai':
            return AIPlayer(num, debug)
        elif name == 'random':
...
if __name__ == '__main__':
    ...
    main(args.player1, args.player2, args.time, args.debug)
